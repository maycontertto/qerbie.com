...
export async function startCarwashCustomerSession(formData: FormData) {
  ...
  if (name.length < 2) {
    redirect(`/l/${encodeURIComponent(qrToken)}?error=invalid_name`);
  }
  ...
  redirect(`/l/${encodeURIComponent(qrToken)}/menu`);
}
