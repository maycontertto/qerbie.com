...
export async function startPetCustomerSession(formData: FormData) {
  ...
  if (name.length < 2) {
    redirect(`/p/${encodeURIComponent(qrToken)}?error=invalid_name`);
  }
  ...
  redirect(`/p/${encodeURIComponent(qrToken)}/menu`);
}
