...
export async function startBeautyCustomerSession(formData: FormData) {
  ...
  if (name.length < 2) {
    redirect(`/s/${encodeURIComponent(qrToken)}?error=invalid_name`);
  }
  ...
  redirect(`/s/${encodeURIComponent(qrToken)}/menu`);
}
