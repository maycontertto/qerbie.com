...
async function requireAestheticAccess() {
...
export async function createAestheticQrToken(formData: FormData): Promise<void> {
  const label = (formData.get("label") as string | null)?.trim() ?? "";
  const returnTo = safeReturnTo(formData.get("return_to"), ["/dashboard/modulos/estetica_qr"]);

  const finalLabel = label.length >= 1 ? label.slice(0, 80) : "Clínica de Estética";

  const { supabase, merchant } = await requireAestheticAccess();

  const qrToken = randomBytes(18).toString("base64url");
  const { error } = await supabase.from("aesthetic_qr_tokens").insert({
...
export async function deactivateAestheticQrToken(formData: FormData): Promise<void> {
...
export async function createAestheticService(formData: FormData): Promise<void> {
  const name = (formData.get("name") as string | null)?.trim() ?? "";
  const description = (formData.get("description") as string | null)?.trim() ?? "";
  const importantNotes = (formData.get("important_notes") as string | null)?.trim() ?? "";
  ...
  const returnTo = safeReturnTo(formData.get("return_to"), ["/dashboard/modulos/estetica_servicos"]);
  ...
  const { supabase, merchant } = await requireAestheticAccess();
  const { error } = await supabase.from("aesthetic_services").insert({
    merchant_id: merchant.id,
    name: name.slice(0, 120),
    description: description ? description.slice(0, 800) : null,
    important_notes: importantNotes ? importantNotes.slice(0, 800) : null,
...
export async function updateAestheticService(formData: FormData): Promise<void> {
  const id = (formData.get("id") as string | null)?.trim() ?? "";
  const name = (formData.get("name") as string | null)?.trim() ?? "";
  const description = (formData.get("description") as string | null)?.trim() ?? "";
  const importantNotes = (formData.get("important_notes") as string | null)?.trim() ?? "";
...
export async function updateAestheticProfessionalServices(formData: FormData): Promise<void> {
  const queueId = (formData.get("queue_id") as string | null)?.trim() ?? "";
  const returnTo = safeReturnTo(formData.get("return_to"), ["/dashboard/modulos/estetica_profissionais"]);
  ...
  if (uniqueServiceIds.length) {
    ...
    if (rows.length) {
      const { error } = await supabase.from("aesthetic_queue_services").insert(rows);
      if (error) {
        redirect(`${returnTo}?error=save_failed`);
      }
    }
