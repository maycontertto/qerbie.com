...
export async function startAestheticCustomerSession(formData: FormData) {
  ...
  if (name.length < 2) {
    redirect(`/e/${encodeURIComponent(qrToken)}?error=invalid_name`);
  }
  ...
  redirect(`/e/${encodeURIComponent(qrToken)}/menu`);
}
