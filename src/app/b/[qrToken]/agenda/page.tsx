...
import { CustomerBarbershopAgendaBrowser } from "@/app/b/[qrToken]/agenda/CustomerBarbershopAgendaBrowser";

export default async function BarbeariaAgendaPage({
  params,
}: {
  ...
  const [{ data: queues }, { data: slots }, { data: services }] = await Promise.all([
    supabase
    ...
    <div className="min-h-screen bg-zinc-50 px-4 py-10 dark:bg-zinc-950">
      <div className="mx-auto w-full max-w-md space-y-6">
        ...
        {!services?.length ? (
          ...
          <CustomerBarbershopAgendaBrowser
            qrToken={qrToken}
            slots={slots ?? []}
            queues={queues ?? []}
            services={services ?? []}
