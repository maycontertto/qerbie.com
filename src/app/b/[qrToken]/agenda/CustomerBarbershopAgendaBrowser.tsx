...
function storageKey(qrToken: string): string {
  return `qerbie_barbershop_agenda_request:${qrToken}`;
}
...
export function CustomerBarbershopAgendaBrowser({
  qrToken,
  slots,
  queues,
  services,
  ...
  const refreshRequest = useCallback(
    async (requestId: string): Promise<RefreshResult> => {
      const res = await fetch(
        `/api/b/${encodeURIComponent(qrToken)}/agenda/requests/${encodeURIComponent(requestId)}`,
        { cache: "no-store" },
      );
  ...
  async function requestSlot(slotId: string) {
    ...
    try {
      const res = await fetch(`/api/b/${encodeURIComponent(qrToken)}/agenda/requests`, {
        method: "POST",
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Escolha um horário disponível e envie sua solicitação. A barbearia confirma em seguida.
        </p>
        ...
        {requestStatusText ? (
          <div className="mt-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
            ...
            {request?.slotStartsAt && request?.slotEndsAt ? (
              ...
              <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Serviço: {selectedService.name}</div>
            ) : null}
          </div>
        ) : null}
      </div>
