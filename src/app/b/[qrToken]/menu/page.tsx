...
export default async function BarbeariaMenuPage({
  params,
}: {
  ...
  if (!token) {
    return <CustomerInvalidQr backHref={`/b/${encodeURIComponent(qrToken)}`} />;
  }
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-brand/5 via-zinc-50 to-white px-4 py-10 dark:from-brand/10 dark:via-zinc-950 dark:to-zinc-950">
      <div className="mx-auto w-full max-w-md space-y-6">
        ...
        {!hasSession ? (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/40 dark:text-amber-200">
            ...
            <div className="mt-2">
              <Link href={`/b/${encodeURIComponent(qrToken)}`} className="font-semibold hover:underline">
                Começar
              </Link>
            </div>
          </div>
        ...
        <div className="grid gap-3">
          <a
            href={`/b/${encodeURIComponent(qrToken)}/fila`}
          ...
          >
            <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Entrar na fila</div>
            <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Escolha o serviço e o barbeiro (se preferir).
            </div>
          </a>

          <a
            href={`/b/${encodeURIComponent(qrToken)}/agenda`}
            className="rounded-2xl border border-zinc-200 bg-white p-4 text-left shadow-sm hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:bg-zinc-800"
          >
            <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Agendar horário</div>
            <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Veja horários disponíveis e envie sua solicitação.
            </div>
          </a>
        </div>
        ...
        <div className="text-center">
          <a
            href={`/b/${encodeURIComponent(qrToken)}`}
      ...
      <CustomerServiceAssistant vertical="b" qrToken={qrToken} />
    </div>
  );
}
