...
function normalizePetName(input: string): string {
  const v = input.trim();
  if (v.length < 2) return "";
  return v.slice(0, 80);
}
...
export async function POST(
  ...
  const customerName = normalizeName(typeof body?.customerName === "string" ? body.customerName : defaultName);
  const petName = normalizePetName(typeof body?.petName === "string" ? body.petName : "");
