...
import { CustomerCarwashQueueBrowser } from "./CustomerCarwashQueueBrowser";

export default async function LavaJatoFilaPage({
  params,
}: {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 px-4 py-10 dark:bg-zinc-950">
      <div className="mx-auto w-full max-w-md space-y-6">
        <Link
          href={`/l/${encodeURIComponent(qrToken)}/menu`}
        ...
        {!services?.length ? (
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
            Nenhum serviço ativo cadastrado. Peça para o lava jato cadastrar os serviços.
          </div>
        ) : !queues?.length ? (
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
            Nenhum profissional/fila ativa cadastrada. Peça para o lava jato criar filas (Recepção).
          </div>
        ) : (
          <CustomerCarwashQueueBrowser
            qrToken={qrToken}
            queues={queues ?? []}
            services={services ?? []}
