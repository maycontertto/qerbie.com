...
function storageKey(qrToken: string, queueId: string): string {
  return `qerbie_carwash_queue_ticket:${qrToken}:${queueId}`;
}
...
export function CustomerCarwashQueueBrowser({
  qrToken,
  queues,
  services,
  queueServices,
  ...
  const refreshTicket = useCallback(
    async (queueId: string, ticketId: string): Promise<RefreshTicketResult> => {
      const res = await fetch(
        `/api/l/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(queueId)}/tickets/${encodeURIComponent(ticketId)}`,
        { cache: "no-store" },
      );
  ...
  async function joinQueue() {
    if (!hasSession) {
      window.location.href = `/l/${encodeURIComponent(qrToken)}`;
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/l/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(selectedQueueId)}/tickets`,
        {
          method: "POST",
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Selecione o serviço e confirme sua presença para entrar na fila.
        </p>
        ...
        <div className="mt-4 grid gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Equipe</label>
            <select
            ...
            {eligibleQueueIds && eligibleQueues.length === 0 ? (
              <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">
                Este serviço ainda não foi vinculado a nenhuma equipe.
              </p>
            ) : null}
          </div>

          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Veículo (opcional)</label>
            <input
              value={vehicleLabel}
              onChange={(e) => setVehicleLabel(e.target.value)}
              placeholder="Ex: Civic preto / ABC-1234"
      ...
      {ticket ? (
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Seu atendimento</p>
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Status: {statusText ?? ""}</p>

          <div className="mt-4 grid grid-cols-3 gap-3">
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-950">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Senha</p>
              <p className="mt-1 text-xl font-bold text-zinc-900 dark:text-zinc-50">{ticket.ticketNumber}</p>
            </div>
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-950">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Posição</p>
              <p className="mt-1 text-xl font-bold text-zinc-900 dark:text-zinc-50">{ticket.position || "—"}</p>
            </div>
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-950">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Tempo</p>
              <p className="mt-1 text-xl font-bold text-zinc-900 dark:text-zinc-50">
                {ticket.etaMinutes === null ? "—" : formatEta(ticket.etaMinutes)}
              </p>
            </div>
          </div>

          <p className="mt-4 text-xs text-zinc-500 dark:text-zinc-400">
            Acompanhe o status aqui. Quando for chamado, se dirija ao atendimento.
          </p>
        </div>
      ) : null}
    </div>
