...
function storageKey(qrToken: string): string {
  return `qerbie_carwash_agenda_request:${qrToken}`;
}
...
export function CustomerCarwashAgendaBrowser({
  qrToken,
  slots,
  queues,
  services,
  ...
  const refreshRequest = useCallback(
    async (requestId: string): Promise<RefreshResult> => {
      const res = await fetch(
        `/api/l/${encodeURIComponent(qrToken)}/agenda/requests/${encodeURIComponent(requestId)}`,
        { cache: "no-store" },
      );
      ...
      return {
        ...
        data: {
          ...
          vehicleLabel: typeof json.vehicleLabel === "string" ? json.vehicleLabel : null,
        },
      };
    },
    [qrToken],
  ...
  async function requestSlot(slotId: string) {
    ...
    try {
      const res = await fetch(`/api/l/${encodeURIComponent(qrToken)}/agenda/requests`, {
        method: "POST",
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Escolha um serviço, uma equipe (opcional) e um horário disponível.
        </p>
        ...
        <div className="mt-4 grid gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Equipe (opcional)</label>
            <select
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Ex: preferência de horário, detalhes"
      ...
      {request ? (
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            {request.queueId ? ` • Equipe: ${queueNameById.get(request.queueId) ?? ""}` : null}
            {request.vehicleLabel ? ` • Veículo: ${request.vehicleLabel}` : null}
          </p>
        </div>
      ) : null}
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Selecione um horário para solicitar.</p>
            ...
            filteredSlots.map((s) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="font-semibold text-zinc-900 dark:text-zinc-50">{formatDateTime(s.starts_at)}</span>
