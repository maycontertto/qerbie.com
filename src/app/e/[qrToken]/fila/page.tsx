...
import { CustomerAestheticQueueBrowser } from "@/app/e/[qrToken]/fila/CustomerAestheticQueueBrowser";

export default async function EsteticaFilaPage({
  params,
}: {
  ...
  const [{ data: queues }, { data: services }, { data: mapRows }] = await Promise.all([
    ...
    supabase
      ...
      .select("id, name, price_cents, duration_min, important_notes")
      .eq("merchant_id", token.merchant_id)
      .eq("is_active", true)
  ...
  return (
    <div className="min-h-screen bg-zinc-50 px-4 py-10 dark:bg-zinc-950">
      <div className="mx-auto w-full max-w-md space-y-6">
        <Link
          href={`/e/${encodeURIComponent(qrToken)}/menu`}
        ...
        {!services?.length ? (
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
            Nenhum procedimento ativo cadastrado. Peça para a clínica cadastrar os serviços.
          </div>
        ) : !queues?.length ? (
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
            Nenhum profissional/fila ativa cadastrada. Peça para a clínica criar filas (Recepção).
          </div>
        ) : (
          <CustomerAestheticQueueBrowser
            qrToken={qrToken}
            queues={queues ?? []}
            services={services ?? []}
