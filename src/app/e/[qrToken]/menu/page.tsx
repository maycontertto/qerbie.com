...
export default async function EsteticaMenuPage({
  params,
}: {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 px-4 py-10 dark:bg-zinc-950">
      <div className="mx-auto w-full max-w-md space-y-6">
        ...
        {!hasSession ? (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/40 dark:text-amber-200">
            ...
            <div className="mt-2">
              <Link href={`/e/${encodeURIComponent(qrToken)}`} className="font-semibold hover:underline">
                Começar
              </Link>
            </div>
          </div>
        ...
        <div className="grid gap-3">
          <a
            href={`/e/${encodeURIComponent(qrToken)}/fila`}
        ...
        <div className="text-center">
          <a
            href={`/e/${encodeURIComponent(qrToken)}`}
      ...
      <CustomerServiceAssistant vertical="e" qrToken={qrToken} />
    </div>
  );
}
