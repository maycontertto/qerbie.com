...
function storageKey(qrToken: string, queueId: string): string {
  return `qerbie_beauty_queue_ticket:${qrToken}:${queueId}`;
}
...
export function CustomerBeautyQueueBrowser({
  qrToken,
  queues,
  services,
  queueServices,
  ...
  const refreshTicket = useCallback(
    async (queueId: string, ticketId: string): Promise<RefreshTicketResult> => {
      const res = await fetch(
        `/api/s/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(queueId)}/tickets/${encodeURIComponent(ticketId)}`,
        { cache: "no-store" },
      );
  ...
  async function joinQueue() {
    ...
    try {
      const res = await fetch(
        `/api/s/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(selectedQueueId)}/tickets`,
        {
          method: "POST",
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            disabled={
              ...
              !selectedQueueId ||
              !selectedServiceId ||
              (eligibleQueueIds ? eligibleQueues.length === 0 : false)
            }
            onClick={joinQueue}
      ...
      {ticket ? (
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400">Sua senha</p>
              <p className="mt-1 text-3xl font-bold text-zinc-900 dark:text-zinc-50">{ticket.ticketNumber}</p>
            </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-950">
              <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400">Posição</p>
              <p className="mt-1 text-lg font-semibold text-zinc-900 dark:text-zinc-50">{ticket.position}</p>
            </div>
            <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-950">
              <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400">Previsão</p>
          ...
          <p className="mt-4 text-xs text-zinc-500 dark:text-zinc-400">
            Mantenha esta tela aberta. A posição e status atualizam automaticamente.
          </p>
        </div>
      ) : null}
    </div>
