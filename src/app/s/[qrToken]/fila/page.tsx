...
import { CustomerBeautyQueueBrowser } from "@/app/s/[qrToken]/fila/CustomerBeautyQueueBrowser";

export default async function SalaoFilaPage({
  params,
}: {
    ...
    <div className="min-h-screen bg-zinc-50 px-4 py-10 dark:bg-zinc-950">
      <div className="mx-auto w-full max-w-md space-y-6">
        ...
        {!services?.length ? (
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
            Nenhum profissional/fila ativo cadastrado. Peça para o salão cadastrar a equipe.
          </div>
        ) : (
          <CustomerBeautyQueueBrowser
            qrToken={qrToken}
            queues={queues ?? []}
            services={services ?? []}
