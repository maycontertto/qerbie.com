...
import { createBarbershopQrToken, deactivateBarbershopQrToken } from "@/lib/barbershop/actions";
import { TableQrPanel } from "@/app/dashboard/modulos/mesas/TableQrPanel";

export default async function BarbeariaQrPage({
  searchParams,
}: {
  ...
  const activeCustomerUrl = activeQrToken ? `${origin}/b/${encodeURIComponent(activeQrToken)}` : null;
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
              Cliente escaneia e escolhe fila/agendamento.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white/70 p-5 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
                ...
                <form action={createBarbershopQrToken} className="mt-3 space-y-3">
                  <input type="hidden" name="return_to" value="/dashboard/modulos/barbearia_qr" />
                  <input
                    name="label"
                    type="text"
                    ...
                    {(tokens ?? []).map((t) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          {t.is_active ? (
                            <form action={deactivateBarbershopQrToken}>
                              <input type="hidden" name="return_to" value="/dashboard/modulos/barbearia_qr" />
                          ...
                          <a
                            href={`/dashboard/modulos/barbearia_qr?qr=${encodeURIComponent(t.qr_token)}`}
                          ...
                          <a href={`${origin}/b/${encodeURIComponent(t.qr_token)}`} className="text-xs font-semibold text-zinc-900 hover:underline dark:text-zinc-50">
                            Abrir
                          </a>
                        </div>
                      </li>
