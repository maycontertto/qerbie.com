...
export default async function PetServicosPage({
  searchParams,
}: {
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Serviços do pet shop, valores e duração.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[360px_1fr]">
          <aside className="rounded-2xl border border-zinc-200 bg-white/70 p-5 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <form action={createPetService} className="mt-4 space-y-3">
              <input type="hidden" name="return_to" value="/dashboard/modulos/pet_servicos" />
              <input
                name="name"
                required
                minLength={2}
            ...
            {items?.length ? (
              items.map((s) => (
                ...
                >
                  ...
                  <form action={updatePetService} className="mt-4 grid gap-3 sm:grid-cols-2">
                    <input type="hidden" name="return_to" value="/dashboard/modulos/pet_servicos" />
