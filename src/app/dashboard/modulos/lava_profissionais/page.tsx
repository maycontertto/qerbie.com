...
import { updateCarwashProfessionalServices } from "@/lib/carwash/actions";

export default async function LavaProfissionaisPage({
  searchParams,
}: {
      ...
      <div className="min-h-screen">
        <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
          <div className="rounded-2xl border border-zinc-200 bg-white/70 p-8 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <h1 className="mt-4 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Equipe</h1>
  ...
  const [{ data: queues }, { data: services }, { data: mappings }] = await Promise.all([
    ...
    supabase.from("carwash_queue_services").select("queue_id, service_id").eq("merchant_id", merchant.id),
  ]);
    ...
    <div className="min-h-screen">
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Equipe</h1>
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Vincule quais serviços cada funcionário executa.</p>
        </div>
        ...
        <section className="mt-8 space-y-4">
          {!queues?.length ? (
            <div className="rounded-2xl border border-zinc-200 bg-white/70 p-6 text-sm text-zinc-600 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-zinc-300">
              Nenhum funcionário/fila ativo cadastrado ainda. Crie filas no módulo Recepção.
            </div>
          ) : !services?.length ? (
            <div className="rounded-2xl border border-zinc-200 bg-white/70 p-6 text-sm text-zinc-600 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-zinc-300">
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Selecione os serviços que esta equipe atende.</p>
                  </div>

                  <form action={updateCarwashProfessionalServices} className="mt-4 space-y-4">
                    <input type="hidden" name="return_to" value="/dashboard/modulos/lava_profissionais" />
