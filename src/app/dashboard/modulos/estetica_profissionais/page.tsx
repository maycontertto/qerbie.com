...
import { updateAestheticProfessionalServices } from "@/lib/aesthetic/actions";

export default async function EsteticaProfissionaisPage({
  searchParams,
}: {
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Vincule quais procedimentos cada profissional realiza.
          </p>
        </div>
        ...
        <section className="mt-8 space-y-4">
          {!queues?.length ? (
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white/70 p-6 text-sm text-zinc-600 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-zinc-300">
              Nenhum procedimento cadastrado ainda. Cadastre serviços primeiro.
            </div>
          ) : (
            (queues ?? []).map((q) => {
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                        Selecione os procedimentos que este profissional atende.
                      </p>
                    </div>
                  </div>

                  <form action={updateAestheticProfessionalServices} className="mt-4 space-y-4">
                    <input type="hidden" name="return_to" value="/dashboard/modulos/estetica_profissionais" />
