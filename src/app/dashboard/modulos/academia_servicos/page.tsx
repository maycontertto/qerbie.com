...
export default async function AcademiaServicosPage({
  searchParams,
}: {
      ...
      <div className="min-h-screen">
        <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
          <div className="rounded-2xl border border-zinc-200 bg-white/70 p-8 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <h1 className="mt-4 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Serviços adicionais</h1>
  ...
  const { data: items } = await supabase
    ...
    .select("id, name, price_cents, is_active, updated_at")
    ...
    <div className="min-h-screen">
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Serviços adicionais</h1>
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cadastre extras opcionais.</p>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[360px_1fr]">
          <aside className="rounded-2xl border border-zinc-200 bg-white/70 p-5 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <form action={createGymAdditionalService} className="mt-4 space-y-3">
              <input type="hidden" name="return_to" value="/dashboard/modulos/academia_servicos" />
              <input
                name="name"
                required
                minLength={2}
            ...
            {items?.length ? (
              items.map((s) => (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                        {formatBrlCents(Number(s.price_cents ?? 0))}
                      </p>
                    </div>
                    <span
                  ...
                  <form action={updateGymAdditionalService} className="mt-4 grid gap-3 sm:grid-cols-2">
                    <input type="hidden" name="return_to" value="/dashboard/modulos/academia_servicos" />
