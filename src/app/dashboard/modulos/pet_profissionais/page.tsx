...
import { updatePetProfessionalServices } from "@/lib/pet/actions";

export default async function PetProfissionaisPage({
  searchParams,
}: {
  ...
  const [{ data: queues }, { data: services }, { data: mappings }] = await Promise.all([
    ...
    supabase.from("pet_queue_services").select("queue_id, service_id").eq("merchant_id", merchant.id),
  ]);
            ...
            (queues ?? []).map((q) => {
              ...
              return (
                ...
                >
                  ...
                  <form action={updatePetProfessionalServices} className="mt-4 space-y-4">
                    <input type="hidden" name="return_to" value="/dashboard/modulos/pet_profissionais" />
