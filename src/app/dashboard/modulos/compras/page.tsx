...
export default async function ComprasModulePage({
  ...
  searchParams: Promise<{ saved?: string; error?: string; review_invoice?: string }>;
}) {
  const { saved, error, review_invoice: reviewInvoiceId } = await searchParams;
  if (reviewInvoiceId) redirect(`/dashboard/modulos/notas_fiscais?review_invoice=${encodeURIComponent(reviewInvoiceId)}`);
    ...
    <div className="min-h-screen">
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        ...
        <div className="flex flex-wrap gap-3 text-sm">
          <Link href="/dashboard/modulos/notas_fiscais" className="font-medium text-zinc-900 hover:underline dark:text-zinc-50">
            Notas recebidas
          </Link>
