...
import { createBeautyService, updateBeautyService } from "@/lib/beauty/actions";

function formatBrlCents(cents: number): string {
  return new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format((cents ?? 0) / 100);
}

export default async function SalaoServicosPage({
  searchParams,
}: {
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Serviços do salão, valores e duração.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[360px_1fr]">
          <aside className="rounded-2xl border border-zinc-200 bg-white/70 p-5 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <form action={createBeautyService} className="mt-4 space-y-3">
              <input type="hidden" name="return_to" value="/dashboard/modulos/salao_servicos" />
              <input
                name="name"
                required
                minLength={2}
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      {s.notes ? (
                        <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">{s.notes}</p>
                      ) : null}
                      {s.image_url ? (
                  ...
                  <form action={updateBeautyService} className="mt-4 grid gap-3 sm:grid-cols-2">
                    <input type="hidden" name="return_to" value="/dashboard/modulos/salao_servicos" />
