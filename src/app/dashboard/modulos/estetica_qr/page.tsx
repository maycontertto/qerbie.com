...
import { createAestheticQrToken, deactivateAestheticQrToken } from "@/lib/aesthetic/actions";
import { TableQrPanel } from "@/app/dashboard/modulos/mesas/TableQrPanel";

export default async function EsteticaQrPage({
  searchParams,
}: {
  ...
  const activeCustomerUrl = activeQrToken ? `${origin}/e/${encodeURIComponent(activeQrToken)}` : null;
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cliente escolhe procedimento e faz fila/agendamento.</p>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white/70 p-5 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
                ...
                <form action={createAestheticQrToken} className="mt-3 space-y-3">
                  <input type="hidden" name="return_to" value="/dashboard/modulos/estetica_qr" />
                  <input
                    name="label"
                    type="text"
                    defaultValue="Clínica de Estética"
                    ...
                    {(tokens ?? []).map((t) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          {t.is_active ? (
                            <form action={deactivateAestheticQrToken}>
                              <input type="hidden" name="return_to" value="/dashboard/modulos/estetica_qr" />
                          ...
                          <a
                            href={`/dashboard/modulos/estetica_qr?qr=${encodeURIComponent(t.qr_token)}`}
                            aria-current={t.qr_token === activeQrToken ? "page" : undefined}
                            className={`text-xs font-semibold hover:underline ${
                              t.qr_token === activeQrToken ? "text-zinc-400 dark:text-zinc-500" : "text-zinc-600 dark:text-zinc-300"
                            }`}
                          >
                            Ver QR
                          </a>

                          <a href={`${origin}/e/${encodeURIComponent(t.qr_token)}`} className="text-xs font-semibold text-zinc-900 hover:underline dark:text-zinc-50">
                            Abrir
                          </a>
                        </div>
                      </li>
