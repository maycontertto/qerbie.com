...
import { createCarwashService, updateCarwashService } from "@/lib/carwash/actions";

function formatBrlCents(cents: number): string {
  return new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format((cents ?? 0) / 100);
}

export default async function LavaServicosPage({
  searchParams,
}: {
  ...
  const canCatalog =
    isOwner || (membership ? hasMemberPermission(membership.role, membership.permissions, "dashboard_products") : false);
    ...
    <div className="min-h-screen">
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Lavagens, cuidados e estética automotiva.</p>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[360px_1fr]">
          <aside className="rounded-2xl border border-zinc-200 bg-white/70 p-5 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60">
            ...
            <form action={createCarwashService} className="mt-4 space-y-3">
              <input type="hidden" name="return_to" value="/dashboard/modulos/lava_servicos" />
              <input
                name="name"
                required
                minLength={2}
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      {s.notes ? <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">{s.notes}</p> : null}
                      {s.image_url ? <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">Foto: {s.image_url}</p> : null}
                    </div>
                    <span
                  ...
                  <form action={updateCarwashService} className="mt-4 grid gap-3 sm:grid-cols-2">
                    <input type="hidden" name="return_to" value="/dashboard/modulos/lava_servicos" />
