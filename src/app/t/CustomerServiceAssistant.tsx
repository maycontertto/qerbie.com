...
/**
 * Widget de chat flutuante compartilhado pelas 5 verticais de serviço
 * (barbearia/estética/lava-jato/pet/salão) - versão simplificada do
 * AssistantWidget.tsx do painel do lojista (sem histórico persistido, sem
 * ações pendentes, só leitura). Ver CustomerMenuAssistant.tsx (vertical `t`,
 * restaurante) para a versão equivalente baseada em cardápio/produtos.
 */
export function CustomerServiceAssistant({
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/public/service-assistant/${vertical}/${encodeURIComponent(qrToken)}`, {
        method: "POST",
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 flex h-[70vh] max-h-150 w-[90vw] max-w-sm flex-col rounded-2xl border border-zinc-200 bg-white/95 shadow-xl backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/95">
          <div className="border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
            <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Assistente</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Pergunte sobre os serviços e preços</p>
          </div>

          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
            {messages.length === 0 && (
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Ex.: &ldquo;quais serviços vocês têm?&rdquo;, &ldquo;quanto custa?&rdquo;
              </p>
            )}
            {messages.map((m, i) => (
