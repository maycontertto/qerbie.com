...
import { CustomerAgendaBrowser } from "./CustomerAgendaBrowser";

export default async function CustomerAgendaPage({
  params,
}: {
  ...
  const [{ data: queues }, { data: slots }] = await Promise.all([
    supabase
      ...
      <div className="mx-auto w-full max-w-2xl space-y-6">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">Agenda</p>
          </div>
        ...
        {!hasSession && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
            <p className="font-semibold">Digite seu nome para solicitar um horário</p>
            <p className="mt-1 text-xs opacity-90">
              A solicitação fica vinculada ao seu dispositivo.
            </p>
            <Link
