...
function storageKey(qrToken: string, queueId: string): string {
  return `qerbie_queue_ticket:${qrToken}:${queueId}`;
}
...
export function CustomerQueueBrowser({
  qrToken,
  queues,
  hasSession,
}: {
  ...
  // Load ticket from localStorage when switching queues.
  useEffect(() => {
    ...
    try {
      ...
      // Start with minimal state; real status comes from polling.
      setTicket({
        ticketId,
        ticketNumber,
        status: "waiting",
  ...
  const refreshTicket = useCallback(
    async (queueId: string, ticketId: string): Promise<RefreshTicketResult> => {
      const res = await fetch(
        `/api/t/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(queueId)}/tickets/${encodeURIComponent(ticketId)}`,
        { cache: "no-store" },
      );
      ...
      const etaMinutes =
        etaRaw === null || etaRaw === undefined ? null : Number(etaRaw);
  ...
  // Poll ticket status every 4s while we have one.
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (out.ok) {
        ...
        // If ticket is terminal, clear from device.
  ...
  async function joinQueue() {
    ...
    try {
      const res = await fetch(
        `/api/t/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(selectedQueueId)}/tickets`,
        {
          method: "POST",
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Escolha o profissional/especialidade e acompanhe sua posição em tempo real.
        </p>

        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Fila</label>
            <select
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Seu nome</label>
            <input
              value={customerName}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <button
            type="button"
            onClick={joinQueue}
            disabled={!selectedQueueId || joining}
            className="rounded-xl bg-zinc-900 px-4 py-2.5 text-xs font-semibold text-white hover:bg-zinc-800 disabled:opacity-60 dark:bg-zinc-50 dark:text-zinc-900 dark:hover:bg-zinc-200"
          >
            {joining ? "Entrando..." : "Entrar na fila"}
          </button>

          {selectedQueue?.avg_service_min ? (
            <span className="text-xs text-zinc-500 dark:text-zinc-400">
              Tempo médio: {selectedQueue.avg_service_min} min
            </span>
          ) : null}
        </div>
      ...
      {ticket ? (
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Seu ticket</p>
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Status: {statusText}</div>
            </div>

            {ticket.status === "waiting" ? (
              <div className="text-right">
                <div className="text-xs text-zinc-500 dark:text-zinc-400">Sua posição</div>
                <div className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">
                  {ticket.position || "—"}
                </div>
              </div>
          ...
          {ticket.status === "waiting" ? (
            <div className="mt-4 rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300">
              {ticket.etaMinutes !== null ? (
                <>
                  Estimativa: <span className="font-semibold">{formatEta(ticket.etaMinutes)}</span>
                </>
              ) : (
                <>Estimativa indisponível (a clínica não definiu tempo médio).</>
              )}
            </div>
          ) : ticket.status === "called" ? (
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
              Você foi chamado. Dirija-se à recepção.
            </div>
          ) : ticket.status === "serving" ? (
            <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900 dark:border-blue-900 dark:bg-blue-950 dark:text-blue-100">
