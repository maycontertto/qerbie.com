...
function storageKey(qrToken: string): string {
  return `qerbie_pet_agenda_request:${qrToken}`;
}
...
export function CustomerPetAgendaBrowser({
  qrToken,
  slots,
  queues,
  services,
  ...
  const refreshRequest = useCallback(
    async (requestId: string): Promise<RefreshResult> => {
      const res = await fetch(
        `/api/p/${encodeURIComponent(qrToken)}/agenda/requests/${encodeURIComponent(requestId)}`,
        { cache: "no-store" },
      );
      ...
      return {
        ...
        data: {
          ...
          petName: typeof json.petName === "string" ? json.petName : null,
        },
      };
    },
    [qrToken],
  ...
  async function requestSlot(slotId: string) {
    ...
    if (pet.length < 2) {
      setError("Informe o nome do pet para continuar.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/p/${encodeURIComponent(qrToken)}/agenda/requests`, {
        method: "POST",
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Escolha um serviço, o pet, um profissional (opcional) e um horário disponível.
        </p>
      ...
      {request ? (
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            {request.petName ? ` • Pet: ${request.petName}` : null}
          </p>
        </div>
      ) : null}
            ...
            filteredSlots.map((s) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs text-zinc-500 dark:text-zinc-400">{s.queue_id ? queueNameById.get(s.queue_id) ?? "" : ""}</span>
                </div>
              </button>
            ))
          )}
