...
function storageKey(qrToken: string, queueId: string): string {
  return `qerbie_pet_queue_ticket:${qrToken}:${queueId}`;
}
...
export function CustomerPetQueueBrowser({
  qrToken,
  queues,
  services,
  queueServices,
  ...
  const refreshTicket = useCallback(
    async (queueId: string, ticketId: string): Promise<RefreshTicketResult> => {
      const res = await fetch(
        `/api/p/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(queueId)}/tickets/${encodeURIComponent(ticketId)}`,
        { cache: "no-store" },
      );
  ...
  async function joinQueue() {
    if (!hasSession) {
      window.location.href = `/p/${encodeURIComponent(qrToken)}`;
      return;
    }
    ...
    if (pet.length < 2) {
      setJoinError("Informe o nome do pet para continuar.");
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/p/${encodeURIComponent(qrToken)}/queues/${encodeURIComponent(selectedQueueId)}/tickets`,
        {
          method: "POST",
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Selecione o serviço, informe o pet e confirme sua presença para entrar na fila.
        </p>
        ...
        <div className="mt-4 grid gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300">Nome do pet</label>
            <input
              value={petName}
